...
int GetEpithet(int nCLASS, int nCOUNT);
string GetEpithetDescription(int nFeat);
...
string GetEpithetName(int nFeat);


void main(string sCOMMAND, string sFEAT){
	object oPC = OBJECT_SELF;
	// get what class we're leveling
	int nCLASS;
	if (GetLocalInt(oPC, "GUI_XTRA_CHOICES_CLASS_FOUND")){
		...
		else SetLocalInt(oPC, "GUI_EXTRA_CHOICES_NUMBER", 1);
	...
	if (TestStringAgainstPattern("**START**", sCOMMAND)){
		...
		string sDESC = GetEpithetDescription(nFEAT);
...
int GetRelevantClass(object oPC, string sCOMMAND){
	...
	if (sCOMMAND == "NATURE_WARRIOR_START") return 113;
		...
		if (GetHasFeat(2537, oPC)) return 62; // half outsider fiend wing options
		else if (GetHasFeat(2538, oPC)) return 621; // half outsider Celestial wing options
		else return -1;
	} else if (sCOMMAND == "HO_PATH_START") return 622;//Half-Outsider Path
		...
		return CLASS_TYPE_FEY;
	else if (sCOMMAND == "PSYWAR_START") return 110; //psywar path. This will be changed to psywar power selection and the paths added to reg bonus feats
	else return -1;
}
...
int GetEpithet(int nCLASS, int nCOUNT){
	switch(nCLASS){
		...
		case 106: switch(nCOUNT){ //VampMal level 6 Bonus Feat
...
void PopulateList(object oPC, int nCLASS, int nPAGE, string sSCREEN){
	...
	while (nCOUNT <= 10)
	{
		nEpithet = GetEpithet(nCLASS, nPAGE*10 + nCOUNT);
		if (nEpithet == -1)
		{
		...
		sICON = Get2DAString("feat", "ICON", nEpithet) + ".tga";
		SetGUITexture(oPC, sSCREEN, "CHOICE_ICON_"+IntToString(nCOUNT), sICON);
		sNAME = GetEpithetName(nEpithet);
		SetGUIObjectText(oPC, sSCREEN, "CHOICE_TEXT_"+IntToString(nCOUNT), -1, sNAME);
		SetLocalGUIVariable(oPC, sSCREEN, nCOUNT, IntToString(nEpithet));
		if (GetHasFeat(nEpithet, oPC, TRUE))
...
int CheckEpithet(int nCLASS, object oPC)
{
...
int GetRelevantClass(object oPC, string sCOMMAND) {
	...
	else if (sCOMMAND == "PSYWAR_START") return 110;
	
	

	if ((GetLevelByClass(42, oPC) > 0) && (CheckEpithet(42, oPC) == FALSE)) return 42; //Celestial Envoy
	if ((GetLevelByClass(49, oPC) > 0) && (CheckEpithet(49, oPC) == FALSE)) return 49; //Draconic Heritage
	
	//if ((GetLevelByClass(62, oPC) == 6) && (GetHasFeat(2537, oPC)) && (CheckEpithet(62, oPC) == FALSE)) return 62; //Half-Fiend Wings/Eyes
	//if ((GetLevelByClass(62, oPC) == 6) && (GetHasFeat(2538, oPC)) && (CheckEpithet(621, oPC) == FALSE)) return 621; //Half-Celestial Wings/Eyes, Magic
	...
	//if ((GetLevelByClass(76, oPC) >= 6) && (GetHasFeat(2537, oPC)) && (CheckEpithet(62, oPC) == FALSE)) return 62; //Half-Fiend(magic path) //Wings/Eyes, Magic
	//if ((GetLevelByClass(76, oPC) >= 6) && (GetHasFeat(2538, oPC)) && (CheckEpithet(621, oPC) == FALSE)) return 621; //Half-Celestial(Magic path) Wings/Eyes, Magic

	//if ((GetLevelByClass(104, oPC) > 0) && (CheckEpithet(104, oPC) == FALSE)) return 104; //Lycan Affliction
	...
	//if ((GetLevelByClass(110,oPC) > 0 ) && (CheckEpithet(110,oPC) == FALSE)) return 110;//Psychic Warrior
	//if ((GetLevelByClass(114,oPC) > 0 ) && (CheckEpithet(114,oPC) == FALSE)) return 114;//Half Undead
	if ((GetLevelByClass(49, oPC) >= 6) && (CheckEpithet(499, oPC) == FALSE)) return 499; //Half-Dragon Wings
	if (GetRacialType(oPC) == RACIAL_TYPE_FEY && GetHasFeat(2843, oPC) && (CheckEpithet(CLASS_TYPE_FEY, oPC) == FALSE)) return CLASS_TYPE_FEY;
	else return -1;
}
