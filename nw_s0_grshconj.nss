...
void main()
{
	...
	float fSummonDuration = RoundsToSeconds(nCasterLevel + 3) * 3; // Summon duration is 3 + caster level rounds.
...
void HandleCastOnGround( location lTarget, int nDurType, float fDuration )
{
	...
	int nCasterLevel = GetCasterLevel(OBJECT_SELF);
	effect eSummon = EffectSummonCreature("ps_grshadowconj", VFX_HIT_SPELL_SUMMON_CREATURE);
