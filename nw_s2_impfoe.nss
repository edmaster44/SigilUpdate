...
//	else
	{
		...
		effect eDivine = EffectDamage( 6 , DAMAGE_TYPE_DIVINE);
		...
		//effect eSelfLink	= EffectLinkEffects(eDur);
	    ...
	    while(GetIsObjectValid(oTarget))
	    {
	    	if(spellsIsTarget(oTarget, SPELL_TARGET_ALLALLIES, OBJECT_SELF) && oTarget != OBJECT_SELF)
	    	{
	            ...
	            DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDivineLink, oTarget, fDuration));
