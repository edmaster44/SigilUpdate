...
void main()
{
	...
	// Agony_Aunt: Base duration is 10 rounds
	// cryptc: duration is still kind of short, changing to 90 base seconds, and x3 for bonus
	float fDur     = 90.0; //IntToFloat(18); // 18 seconds/3 rounds
 	
	// Agony_Aunt: Add Blackguard and Assassin levels to duration.
	int iExtraRounds = GetLevelByClass(CLASS_TYPE_ASSASSIN, oPC) + GetLevelByClass(CLASS_TYPE_BLACKGUARD, oPC);
	// Agony_Aunt: As the new "distilled" potions can be created on the fly and just cost jink, for the
	// moment let's test with extending the duration by double the applicable class levels.
	// We multiply by 6 to turn seconds into rounds as well.
	if (iExtraRounds > 0)
	{
		iExtraRounds = iExtraRounds * 6 * 3;
		fDur = fDur + IntToFloat(iExtraRounds);
	}
	...
	*/
	...
	if (!bHasFeat) // If no Handle Poison feat, then 5% chance of failure
	{
        ...
        // Agony_Aunt: No need to check, we don't use this switch
