...
// End of Spell Cast Hook
    ...
    switch (nAlign)
    {
        case ALIGNMENT_EVIL:
		{
            nAlign = GetAlignmentLawChaos(OBJECT_SELF);
			if (nAlign==ALIGNMENT_LAWFUL)
				eSummon = EffectSummonCreature("ps_summon_palawful",VFX_HIT_SPELL_SUMMON_CREATURE, fDelay);
			else
				eSummon = EffectSummonCreature("ps_summon_pachaotic",VFX_HIT_SPELL_SUMMON_CREATURE, fDelay);

            //eGate = EffectVisualEffect(VFX_FNF_SUMMON_GATE);
        	break;
		}
        case ALIGNMENT_GOOD:
            eSummon = EffectSummonCreature("ps_summon_pagood", VFX_HIT_SPELL_SUMMON_CREATURE, fDelay);
            //eGate = EffectVisualEffect(VFX_FNF_SUMMON_CELESTIAL);
        break;
        case ALIGNMENT_NEUTRAL:
            eSummon = EffectSummonCreature("ps_summon_paneutral",VFX_HIT_SPELL_SUMMON_CREATURE, 1.0);
