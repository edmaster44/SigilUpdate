    ...
    if (GetIsObjectValid(oItem))
    {
        if (GetObjectType(oItem) == OBJECT_TYPE_ITEM)
        {
            ...
            if (nId == 510 && GetPlotFlag(oItem) == FALSE) // decharge
            {
                ...
                int currentValue = nPreviousValue+nValue;
                int numCharges = currentValue/4000;
                float percent = (currentValue -(4000*numCharges))/40.0;
                SendMessageToPC(OBJECT_SELF, "Magic Electrifier charges:" + IntToString(numCharges) + ", " + FloatToString(percent, 0, 2) + "% till next charge");
                // End Lawjoskar-added section --
